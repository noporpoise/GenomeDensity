...
sub print_usage
{
  ...
  print STDERR "Usage: ./around_hotpots.pl <hotspots.txt> <SNP|max_indel> " .
               "<chr_sizes.csv> <in.vcf> <out_dir>
  Prints commands to produce density graphs for variants around recombination
  hotspots/coldspots.  \n";
  exit;
}
...
for(my $b = 0; $b < @bin_sizes; $b++)
{
  ...
  run_cmd($bin_size, $num_bins, $hotspot_file);
  ...
  {
    # With no polarity
    run_cmd($bin_size, $num_bins, $hotspot_file, $slippage);
...
sub run_cmd
{
  my ($bin_size, $num_bins, $hotspot_file,
      $slippage, $polarity, $indel_size) = @_;

  my $out_file = $out_dir . '/density.recomb';
  my $run = "";
  my $in = $vcf_file;
  ...
  $run .= "$density_cmd $bin_size $num_bins HOTSPOT $hotspot_file $chr_sizes $out_file $in";
