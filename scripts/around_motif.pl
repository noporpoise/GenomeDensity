...
sub print_usage
{
  ...
  print STDERR "Usage: ./around_ensgene.pl <motif.txt> <max_indel> <chr_sizes.csv> <in.vcf> <out_dir>\n";
...
sub run_cmd
{
  ...
  my $out_file = $out_dir . '/density.motif';
  my $run = "";
  my $in = $vcf_file;
  ...
  $run .= "$density_cmd $bin_size $num_bins MOTIF $motif_file " .
